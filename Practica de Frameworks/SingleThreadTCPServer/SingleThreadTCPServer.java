...
public abstract class SingleThreadTCPServer {
    ...
    protected boolean closeConection(String inputLine) { //	------> IMPLEMENTADO
    	return inputLine.equalsIgnoreCase("");
    }
    
    protected String clientInfo (Socket clientSocket) { // ------> IMPLEMENTADO
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                if (this.closeConection(inputLine)) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            System.out.println("Connection closed with " + this.clientInfo(clientSocket));
        } catch (IOException e) {
